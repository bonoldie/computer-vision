...
def deep_match(reference, target, visibility_reference,  model_pointcloud, match_max_distance=15):
    ...
    for model in deep_match_results['by_model'].keys():
        ...
        model_points = np.asanyarray([*map(lambda index : model_pointcloud_points[visibility_reference[index]['index']], dist_indices[:, 1])])

        print(target_points.shape)
        print(model_points.shape)

        success, r_vec, t_vec = cv2.solvePnP(model_points, target_points, cameraMatrix=dante_camera_matrix, distCoeffs=dante_dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)
        ...
        target_reproj, _ = cv2.projectPoints(model_pointcloud_points, r_vec, t_vec, cameraMatrix=dante_camera_matrix, distCoeffs=dante_dist_coeffs)
        target_reproj = np.squeeze(np.array(target_reproj, dtype=np.float32).astype(int), axis=1)
