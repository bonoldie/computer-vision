...
def evaluateRDD(reference, target, model=None, cache=True):
    ...
    ref_matches, target_matches, conf = model.match(reference, target, resize=1024)
...
#         ref_H,ref_W = ref.shape[:2]
#         ref_LF = cv2.resize(ref, (round(ref_W/LF_scale_factor),round(ref_H/LF_scale_factor)))

#         target_H,target_W = target.shape[:2]
#         target_LF = cv2.resize(ref, (round(target_W/LF_scale_factor),round(target_H/LF_scale_factor)))
...
#         np.save(os.path.join(matches_savepath, 'LiftFeat', 'reference__SAM1005_matches'),ref_matches)
#         np.save(os.path.join(matches_savepath, 'LiftFeat', 'target__SAM1007_matches'),target_matches)
